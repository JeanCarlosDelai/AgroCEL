...
import DeleteCropSaleUseCase from './DeleteCropSaleUseCase';
...
let deleteCropSaleUseCase: DeleteCropSaleUseCase;
let user: IUser;
...
beforeAll(() => {
  ...
  deleteCropSaleUseCase = new DeleteCropSaleUseCase(
    cropsSaleRepositoryInMemory,
  );
});

describe('Delete Crop sale', () => {
  beforeAll(async () => {
  ...
  it('Should be able to delete a crop sale', async () => {
    const expectErrorResponse = new Error('Crop sale not found.');

    await deleteCropSaleUseCase.execute({ id, crop_id });

    expect(cropSale).toHaveProperty('id');

    expect(deleteCropSaleUseCase.execute({ id, crop_id })).rejects.toThrowError(
      expectErrorResponse,
    );
  });
});
