...
import ShowApplicationUseCase from './ShowApplicationUseCase';
...
let showApplicationUseCase: ShowApplicationUseCase;
let user: IUser;
...
beforeAll(() => {
  ...
  showApplicationUseCase = new ShowApplicationUseCase(
    applicationRepositoryInMemory,
  );
});
...
describe('Show Applications', () => {
  ...
  it('Should be able to show application', async () => {
    const response = await showApplicationUseCase.execute({ id, area_id });

    expect(response).toBeTruthy();
    expect(response).toHaveProperty('application_type');
  });

  it('Should not be able to show one application', async () => {
    const id = '1232131321';
    const area_id = '1232131321';
    const expectErrorResponse = new Error('Area or Application not exist.');

    expect(
      showApplicationUseCase.execute({ id, area_id }),
