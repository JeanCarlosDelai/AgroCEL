...
import ListApplicationUseCase from './ListApplicationUseCase';
...
let listApplicationUseCase: ListApplicationUseCase;
let user: IUser;
let property: IProperty;
let area: IArea;
let application: IApplication;
let application2: IApplication;
let area_id: string;
...
beforeAll(() => {
  ...
  listApplicationUseCase = new ListApplicationUseCase(
    applicationRepositoryInMemory,
...
describe('List Crops', () => {
  beforeAll(async () => {
    ...
    const applicationData2: ICreateApplication = {
      ...
      application_date: new Date(),
      application_time: 10,
      description: 'Foi gastado 6 bombas de 300L de água + produto',
    };
    application2 = await createApplicationUseCase.execute(applicationData2);
  });

  it('Should be able to list applications', async () => {
    const response = await listApplicationUseCase.execute(area_id);

    expect(response).toBeTruthy();
    expect(response.data[0].description).toBe(application.description);
    expect(response.data[1].description).toBe(application2.description);
  });
  ...
  it('Should not be able to list crops with invalid area', async () => {
    ...
    expect(listApplicationUseCase.execute(fakePropertyId)).rejects.toThrowError(
      expectErrorResponse,
    );
  });
});
