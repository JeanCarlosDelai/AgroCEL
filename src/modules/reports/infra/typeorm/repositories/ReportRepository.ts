...
class ReportsRepository implements IReportRepository {
  ...
  public async findReport(property_id: string): Promise<IReportsData | null> {
    ...
    const areasWithCrops = await Promise.all(
      areas.map(async (area) => {
        ...
        const cropsWithSalesAndDestinations = await Promise.all(
          crops.map(async (crop) => {
            ...
            const cropDestinations = await this.cropDestinationRepository
              ...
              .where('crop_destination.crop_id = :crop_id', {
                crop_id: crop.id,
              })
              .getMany();
        ...
        return {
          ...area,
          crops: cropsWithSalesAndDestinations,
          applications,
          otherActivities,
        };
      }),
