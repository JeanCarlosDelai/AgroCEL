...
import UpdateCropDestinationUseCase from './UpdateCropDestinationUseCase';
...
let updateCropDestinationUseCase: UpdateCropDestinationUseCase;
let user: IUser;
...
beforeAll(() => {
  ...
  updateCropDestinationUseCase = new UpdateCropDestinationUseCase(
    cropsDestinationRepositoryInMemory,
  );
});

describe('Show Crop destination', () => {
  beforeAll(async () => {
  ...
  it('Should be able to update a crop destination', async () => {
    const cropDerstinationDataUpdate: IUpdateCropDestination = {
      id: id,
      name: 'Coljeita2',
      area_id: area.id,
      crop_id: crop.id,
    ...
    cropDestination = await updateCropDestinationUseCase.execute(
      cropDerstinationDataUpdate,
    );

    expect(cropDestination).toBeTruthy();
    expect(cropDestination.name).toBe(cropDerstinationDataUpdate.name);
  });

  it('Should not be able to update a not exist crop destination', async () => {
    const cropDerstinationDataUpdate: IUpdateCropDestination = {
      id: id,
      name: 'Coljeita2',
      area_id: 'fakeid',
    ...
    expect(
      updateCropDestinationUseCase.execute(cropDerstinationDataUpdate),
