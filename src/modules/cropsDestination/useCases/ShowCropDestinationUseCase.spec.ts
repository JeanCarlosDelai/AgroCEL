...
import ShowCropDestinationUseCase from './ShowCropDestinationUseCase';
...
let showCropDestinationUseCase: ShowCropDestinationUseCase;
let user: IUser;
...
beforeAll(() => {
  ...
  showCropDestinationUseCase = new ShowCropDestinationUseCase(
    cropsDestinationRepositoryInMemory,
  );
});
...
describe('Show Crop destination', () => {
  ...
  it('Should be able to show a crop destination', async () => {
    const response = await showCropDestinationUseCase.execute({ id, crop_id });

    expect(response).toBeTruthy();
    expect(response).toHaveProperty('name');
  });

  it('Should not be able to show one crop destination', async () => {
    const id = '1232131321';
    ...
    expect(
      showCropDestinationUseCase.execute({ id, crop_id }),
