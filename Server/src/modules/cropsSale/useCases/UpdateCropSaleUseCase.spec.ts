...
import UpdateCropSaleUseCase from './UpdateCropSaleUseCase';
...
let updateCropSaleUseCase: UpdateCropSaleUseCase;
let user: IUser;
...
beforeAll(() => {
  ...
  updateCropSaleUseCase = new UpdateCropSaleUseCase(
    cropsSaleRepositoryInMemory,
  );
});

describe('Update Crop sale', () => {
  beforeAll(async () => {
  ...
  it('Should be able to update a crop sale', async () => {
    const cropSaleDataUpdate: IUpdateCropSale = {
      id: id,
      name: 'Venda Natura5',
      area_id: area.id,
      crop_id: crop.id,
    ...
    cropSale = await updateCropSaleUseCase.execute(cropSaleDataUpdate);

    expect(cropSale).toBeTruthy();
    expect(cropSale.name).toBe(cropSaleDataUpdate.name);
  });

  it('Should not be able to update a not exist crop sale', async () => {
    const cropSaleDataUpdate: IUpdateCropSale = {
      id: id,
      name: 'Venda Natura5',
      area_id: 'fakeid',
    ...
    expect(
      updateCropSaleUseCase.execute(cropSaleDataUpdate),
